---
title: 网页 UI 终于能进游戏和 3D 场景了：HTML-in-Canvas 为什么重要
published: 2026-06-23
description: 从用户体验和 Web 渲染架构角度解释 HTML-in-Canvas：真实 DOM 如何保留输入、焦点、无障碍能力，同时作为 Canvas、WebGL、WebGPU 的纹理进入 GPU 合成管线。
tags: [HTML-in-Canvas, Canvas, WebGL, WebGPU, Web 平台, 渲染架构]
category: 前端
draft: false
---
...
2. 绘制/上传方法：2D Canvas 使用 `drawElementImage()`；WebGL 使用 `texElementImage2D()`；WebGPU 通过 `GPUQueue.copyElementImageToTexture()` 上传。
...
这里的关键不是那一次绘制，而是最后一行 transform 同步：浏览器需要知道这个真实 DOM 元素在 Canvas 画面里的位置，才能把点击、焦点、选区和无障碍命中区域对回正确的元素。
