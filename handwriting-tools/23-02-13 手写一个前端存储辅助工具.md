...
在项目开发的过程中，为了减少前端请求，开发者往往需要将一些不易改变的数据放入本地缓存中。如把一些模板数据放入 localStorage 或者 IndexedDB
。代码往往如下书写。
...
可以看到，代码略显冗余，同时这里并没有处理数据版本以及过期时间，同时也没有数据写入功能。如果再把这些功能点加入，代码将会更加复杂。

于是个人写了一个小工具 [storage-tools](https://github.com/wsafight/storage-tools) 来处理这个问题。

## 使用 storage-tools 缓存数据

当前存储默认会使用 localStorage 作为数据源来获取。
...
if (templates === null) {
  api.get("xxx").then((val) => {
    ...
    requestIdleCallback(() => {
      // 闲暇时间再将当前内存的数据存储到 localStorage 中去
      // 再进行提交前都可以进行数据的 存储
      store.commit();
    });
  });
...
const sessionStore = new StorageHelper({
  // 配置同上
  storageKey: `templates.${userId}`,
  version: 1,
  timeout: 60 * 60 * 24,
...
class StorageHelper<T> {
  ...
  upgrade(store: DataStore<T>): DataStore<T> {
    // 获取当前的秒数
    const currentSecond = getCurrentSecond();
    // 看起来很像 getEmptyDataStore 代码，但实际上是不同的义务
...
超时机制很简单，只需要在 getData 的时候检查一下数据即可。
...
class StorageHelper<T> {
  ...
  // 非浏览器环境不具备 localStorage，所以不在此处直接使用
...
如果数据源都是可以同步获取信息的就很方便，但有些数据需要异步构建并获取数据，例如 IndexedDB 。这里我们先建立一个 IndexedDBAdaptor 类。
