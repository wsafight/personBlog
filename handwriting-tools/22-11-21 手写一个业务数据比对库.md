...
处理表单更新对于业务开发来说必不可少。而在前端数据对服务端提交时候，往往需要一个数据比对来剔除不需要更新的数据项。于是个人写了一个业务数据比对库 [diff-helper](https://github.com/wsafight/diff-helper.git)。
...
对比上述结果，可以看到针对对象类型，JSON.stringify 如果不提供 replacer 数组可能会对生成结果产生“误伤”。但从系统实际运行上来说，对象内部很难出现排序异常的情况。所以我们就做如下改造处理。
...
如果只使用 JSON.stringify 话，函数就没有办法灵活的处理用户的各色需求，所以现在可以为当前函数添加用户可以配置的 diff 函数。
...
来优化内部属性处理。该库通过 JSON schema 预先告知对象属性类型，fast-json-stringify 性能。
...
以上所有代码都在 [diff-helper](https://github.com/wsafight/diff-helper.git) 中，大家可以利用传入的参数嵌套处理复杂类型，和服务端搭配必然事半功倍。

我们通过两个函数来处理了对象和数组的变化处理，同时也提供了参数帮助用户可以嵌套处理复杂类型。但实际上对象的比对也是多样的。例如是否能够通过 westore 的 diff 来优化小程序。是否能够利用该方案来优化 mongoDB 的 JSON 数据库等等。
