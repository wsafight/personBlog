...
- **没有表达力天花板**。数据库支持的任何 SQL 特性，你都能直接用——CTE、窗口函数、LATERAL JOIN、递归查询、JSON 操作符。不存在"这个 ORM 支不支持"的问题。
...
v1.25.0 之前，sqlc 通过 cgo 调用 `pganalyze/pg_query_go`——这是 PostgreSQL 源码里的 parser 编译成的 C 库。好处是 100% 兼容 PostgreSQL 语法，坏处是需要 C 编译器，Windows 上很痛苦。

v1.25.0 起，sqlc 切换到 `wasilibs/go-pgquery`——把同一个 C parser 编译成 WebAssembly，通过 `tetratelabs/wazero`（纯 Go 的 WASM 运行时）执行。代价是解析速度慢了约 4-5 倍，但换来了：

- 纯 Go 编译，零 cgo 依赖
- Windows、macOS、Linux 原生支持
- 交叉编译无障碍

**为什么这很重要？** 因为用真实的 PostgreSQL parser 意味着 sqlc 能理解所有 PostgreSQL 语法——不是一个子集，是全部。`CREATE TABLE ... PARTITION BY`、`WITH RECURSIVE`、`LATERAL JOIN`、`jsonb_path_query`——parser 都认识。如果你写了一条语法错误的 SQL，sqlc 会在 `generate` 阶段就告诉你，而不是等到运行时数据库报错。

MySQL 用的是 TiDB 的纯 Go parser（`pingcap/parser`），SQLite 用的是 `modernc.org/sqlite`。每个数据库引擎都有自己的 parser，确保方言级别的正确性。
...
- **参数直接传递**。`q.db.QueryRowContext(ctx, getAuthor, id)` 里的 `id` 就是 `int64`，没有 `interface{}` 装箱（Go 编译器会优化掉）。
- **结果直接 Scan**。`row.Scan(&i.ID, &i.Name, &i.Bio)` 是 `database/sql` 最原始的用法，没有反射、没有 struct tag 查找。
- **没有任何 sqlc 的运行时依赖**。生成的代码只 import `database/sql` 和 `context`。你甚至可以删掉 sqlc，生成的代码照样编译运行。

这就是"零运行时开销"的含义：**生成的代码和你手写的最优代码一模一样**。sqlc 的所有工作都在 `generate` 阶段完成，之后它就消失了。
...
生成的函数接受 `[]int64` 参数，sqlc 在运行时展开成正确数量的占位符。注意：这是 sqlc 少数几个有运行时行为的特性——因为 IN 的参数数量是动态的，没法在编译期确定占位符数量。

### Prepared Statements

```go
q, err := db.Prepare(ctx, db)
if err != nil {
  return err
}
// 后续调用复用预编译的 statement
user, err := q.GetAuthor(ctx, authorID)
```

sqlc 生成的 `Prepare` 函数会一次性预编译所有查询。在连接池场景下，这能显著减少 PostgreSQL 的 parse/plan 开销——第一次执行时 Postgres 缓存 statement，后续同名调用跳过 parse 阶段。
...
rules:
  - name: no-delete-without-where
    ...
    rule: "query.sql MATCHES 'DELETE FROM .* WHERE'"
```
...
**动态查询是最大的痛点**。如果你的业务有大量"根据用户输入动态拼条件"的场景——比如搜索页面有 10 个可选过滤器——sqlc 会很痛苦。你要么写 10 个不同的 SQL 文件，要么用 `CASE WHEN sqlc.narg('name') IS NULL OR name = sqlc.narg('name')` 这种 hack。两种方案都不优雅。这种场景下 squirrel 或者 GORM 的动态查询能力是真正的优势。
...
回头看 sqlc 的整套设计，每个决策都指向同一个方向：

| 决策点 | sqlc 选了啥 | 后果 |
|---|---|---|
| 真相源 | SQL 文件 | 没有表达力天花板，DBA 可直接审查 |
| Parser | PostgreSQL 真实 parser (WASM) | 100% 语法兼容，编译期就能发现语法错误 |
| 类型推导 | 静态分析 + 可选数据库辅助 | 编译期类型安全，nullable 精确处理 |
| 代码生成 | 极简——只生成 struct + 函数 | 生成代码可读、可审计、无框架依赖 |
| 运行时 | 零抽象层 | 性能等同手写 `database/sql` |
| 扩展 | WASM 沙箱插件 | 安全、跨语言、社区可扩展 |
